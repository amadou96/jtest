import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { IRp, Rp } from 'app/shared/model/rp.model';
import { RpService } from './rp.service';
import { RpComponent } from './rp.component';
import { RpDetailComponent } from './rp-detail.component';
import { RpUpdateComponent } from './rp-update.component';

@Injectable({ providedIn: 'root' })
export class RpResolve implements Resolve<IRp> {
  constructor(private service: RpService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IRp> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((rp: HttpResponse<Rp>) => {
          if (rp.body) {
            return of(rp.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Rp());
  }
}

export const rpRoute: Routes = [
  {
    path: '',
    component: RpComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jtest5App.rp.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: RpDetailComponent,
    resolve: {
      rp: RpResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jtest5App.rp.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: RpUpdateComponent,
    resolve: {
      rp: RpResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jtest5App.rp.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: RpUpdateComponent,
    resolve: {
      rp: RpResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jtest5App.rp.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
];
